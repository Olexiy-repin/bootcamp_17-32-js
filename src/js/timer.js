...
const TIMER_DEADLINE = new Date(2022, 4, 30, 13, 38);
...
const timer = {
  ...
  start(rootSelector, deadline) {
    this.intervalId = setInterval(() => {
      ...
      rootSelector.querySelector('.js-timer__days').textContent =
        this.pad(days);
      rootSelector.querySelector('.js-timer__hours').textContent =
        this.pad(hours);
      ...
      rootSelector.querySelector('.js-timer__days').dataset.title =
        this.declensionNum(days, ['день', 'дня', 'дней']);
      rootSelector.querySelector('.js-timer__hours').dataset.title =
        this.declensionNum(hours, ['час', 'часа', 'часов']);
      rootSelector.querySelector('.js-timer__minutes').dataset.title =
        this.declensionNum(minutes, ['минута', 'минуты', 'минут']);
      rootSelector.querySelector('.js-timer__seconds').dataset.title =
        this.declensionNum(seconds, ['секунда', 'секунды', 'секунд']);
    }, 1000);
  },
